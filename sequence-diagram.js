...
alt payment required
  ...
  const init = ({ ce, ca, cv, topMain }) => {
    ...
    const setType = (next, { preserve = true } = {}) => {
      ...
      if (next === 'sequence') {
        state.swimCode = preserve ? state.swimCode : (state.swimCode || ce.value);
        ...
        if (state.type !== 'sequence' || !preserve) state.swimCode = state.swimCode || localStorage.getItem(SWIM_KEY) || '';
    ...
    observer.observe(ca, { attributes: true, childList: true, subtree: false });
    ...
    async function renderSequence() {
      ...
      try {
        ...
        const themeConfig = buildThemeConfig(parsed.theme, rootStyle);
        ...
        const id = `swimflow-sequence-${Date.now()}-${renderId}`;
        const rendered = await window.mermaid.render(id, parsed.mermaid);
        if (renderId !== state.sequenceRenderId) return;
  ...
  function parseSequenceSource(source) {
    ...
    if (config.showSequenceNumbers && !/^\s*autonumber\b/im.test(code)) {
      code = code.replace(/^sequenceDiagram\s*/i, 'sequenceDiagram\n  autonumber\n');
    }
    if (title && !/^\s*title\s+/im.test(code)) {
      code = code.replace(/^sequenceDiagram\s*/i, `sequenceDiagram\n  title ${title}\n`);
    }
  ...
  function normalizeSequenceConfig(input) {
    ...
    Object.entries(input).forEach(([key, value]) => { if (allowed.has(key)) defaults[key] = value; });
    return defaults;
  }

  function buildThemeConfig(theme, css) {
  ...
  function createSuggestions(ce) {
    ...
    const items = [
      ['Participant', 'participant API as API Gateway'],
      ['Actor', 'actor User as Customer'],
      ['Message', 'App ->> API: Request'],
      ['Return', 'API -->> App: Response'],
    ...
    items.forEach(([label, text]) => {
      ...
      button.type = 'button';
      button.className = 'seqSuggestion';
  ...
  function insertAtCursor(ce, text) {
    ...
    const before = ce.value.slice(0, start);
    const after = ce.value.slice(end);
  ...
  function ensureMermaid() {
    ...
    window.__swimflowMermaidPromise = new Promise((resolve, reject) => {
      ...
      script.defer = true;
      script.dataset.swimflowMermaid = 'true';
  ...
  function exportSvg(svgText, title) {
    ...
    downloadBlob(blob, `${safeName(title || 'sequence-diagram')}.svg`);
  }
  ...
  async function exportPng(svgText, title, toClipboard) {
    ...
    try {
      ...
      ctx.fillStyle = light ? '#ffffff' : '#07090e';
      ...
      if (toClipboard && navigator.clipboard && window.ClipboardItem) {
        ...
        showToast('Sequence PNG copied');
      } else {
        downloadBlob(png, `${safeName(title || 'sequence-diagram')}.png`);
      }
    } catch (_) {
  ...
  function getTitle(code) {
    return String(code || '').match(/^\s*diagram\s+["'](.+?)["']/im)?.[1] || 'sequence-diagram';
  }

  function safeName(value) {
    return String(value).trim().toLowerCase().replace(/[^a-z0-9_-]+/g, '-').replace(/^-+|-+$/g, '') || 'diagram';
  }

  function setStatus(message) {
    const el = document.getElementById('st') || document.getElementById('status');
  ...
  function normalizeMermaidError(err) {
    const raw = String(err?.str || err?.message || err || 'Invalid sequence syntax');
    return raw.replace(/Parse error on line\s*/i, 'Syntax error near line ').replace(/mermaid version.*$/im, '').trim();
  }

  function injectStyles() {
    if (document.getElementById('sequenceDiagramStyles')) return;
    const style = document.createElement('style');
    style.id = 'sequenceDiagramStyles';
    style.textContent = `
      #ca { position: relative !important; }
      .diagramTypeSelect { min-width: 105px; margin-left: 5px; font-weight: 600; }
      #sequenceCanvas { position: absolute; inset: 0; overflow: auto; padding: 44px; background: var(--bg); z-index: 8; }
      #sequenceCanvas[hidden] { display: none !important; }
      #sequenceCanvas > svg { display: block; margin: 0 auto; filter: drop-shadow(0 12px 32px rgba(0,0,0,.10)); }
      #sequenceError { position: absolute; top: 16px; left: 50%; transform: translateX(-50%); z-index: 12; max-width: min(720px, calc(100% - 32px)); padding: 10px 14px; border: 1px solid color-mix(in srgb,var(--re) 55%,transparent); border-radius: 9px; background: color-mix(in srgb,var(--re) 10%,var(--p1)); color: var(--re); white-space: pre-wrap; font-size: 10px; box-shadow: 0 12px 30px rgba(0,0,0,.18); }
      #sequenceError[hidden] { display: none !important; }
      .seqEmpty { color: var(--dm); text-align: center; padding-top: 80px; }
      .sequenceSuggestions { width: 100%; display: flex; gap: 4px; align-items: center; overflow-x: auto; padding-top: 2px; scrollbar-width: thin; }
      .sequenceSuggestions[hidden] { display: none !important; }
      .seqSuggestion { flex: 0 0 auto; min-height: 24px; padding: 3px 8px; border-radius: 5px; border: 1px solid var(--b2); background: var(--p1); color: var(--tx); font: 600 9px/1.1 'JetBrains Mono', monospace; cursor: pointer; }
      .seqSuggestion:hover { border-color: var(--ac); color: var(--br); background: var(--p2); }
      #sequenceToast { position: fixed; left: 50%; bottom: 24px; transform: translate(-50%, 12px); opacity: 0; pointer-events: none; z-index: 100001; padding: 8px 12px; border: 1px solid var(--b2); border-radius: 8px; background: var(--p1); color: var(--br); font: 600 10px/1.2 'JetBrains Mono', monospace; box-shadow: 0 10px 28px rgba(0,0,0,.22); transition: opacity .16s, transform .16s; }
      #sequenceToast.show { opacity: 1; transform: translate(-50%, 0); }
      :root[data-ui-theme='light'] #sequenceCanvas { background: #f6f8fc; }
      :root[data-ui-theme='light'] .seqSuggestion { background: #fff; color: #29384d; border-color: #c5d2e1; }
      :root[data-ui-theme='light'] .seqSuggestion:hover { background: #eef4ff; color: #0b1220; border-color: #8fa9c7; }
      @media (max-width: 720px) {
        #sequenceCanvas { padding: 24px 16px 80px; }
        .diagramTypeSelect { min-width: 92px; max-width: 105px; }
      }
    `;
